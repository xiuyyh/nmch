...
import { format, isSameMonth, startOfOfMonth } from "date-fns";
...
export default function ElectricityStatsPage() {
  ...
  const stats = useMemo(() => {
    ...
    const monthlyTotal = expenses
      .filter(e => e.timestamp?.toDate && isSameMonth(e.timestamp.toDate(), now))
    ...
    // Ensure we sort by date before grouping to keep chronological order
            ...
            <Card className="glass-card border-l-4 border-l-primary">
              ...
              <CardContent>
                <div className="flex items-center gap-1 text-[10px] font-bold text-emerald-500 uppercase">
                  <ArrowUpRight className="w-3 h-3" /> {format(new Date(), "MMMM yyyy")}
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="glass-card border-l-4 border-l-amber-500">
              ...
              <CardContent>
                <div className="text-[10px] font-bold text-amber-500 uppercase">
                  Total Impact: ₦{stats?.topApartment?.value.toLocaleString() || 0}
                </div>
              </CardContent>
            </Card>
                ...
                <Card className="glass-card">
                  <CardHeader className="border-b border-white/5 bg-white/[0.02]">
                    <CardTitle className="text-base uppercase flex items-center gap-2">
                      <TrendingUp className="w-4 h-4 text-primary" /> Spend Trend (Recent Months)
                    </CardTitle>
                  </CardHeader>
